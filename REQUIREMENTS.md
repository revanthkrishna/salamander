...
- [ ] Clicking **ok**:
  1. The selection box outline, resize handles, dimming overlay, and comment box are hidden for the single frame of capture (these are drawn within the page area and would otherwise appear in the screenshot). The sidebar itself does **not** need to be hidden — since it resizes the page rather than overlaying it, the page's visible viewport never extends under the sidebar, so the sidebar can never fall inside a selection's crop bounds.
...
3. User clicks a spot on the page → default-sized box appears; user drags handles to resize over the area they want to flag
4. A comment box appears next to the box; user types a note and clicks **ok**
