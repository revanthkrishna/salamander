...
- [ ] User clicks once on the page → a default-sized selection box appears **centered** on the click point (default size: 200×150px, i.e. top-left = `(x - 100, y - 75)`). If centering would push the box outside the viewport, it is clamped by shifting (not shrinking) it fully on-screen, independently per axis — e.g. clicking at (20, 20) would naively center the box at (-80, -55), which clamps to (0, 0), producing a final box of (0,0)–(200,150); the same clamp applies symmetrically near the right/bottom edges.
...
- [ ] Clicking a thumbnail opens it enlarged in a modal (translucent backdrop over the page)
...
- [ ] Comment box appears attached to the selection box as a single rounded, merged surface (textarea + footer bar, no internal padding/border between them), flipping position (below → above → side) based on available viewport space. **Decision:** the confirm button is labelled **save** (was "ok" in v1) and is disabled while the trimmed note is empty; a character counter appears once the note passes 900 characters and turns danger-coloured at 980+. **Rationale:** "save" reads more clearly as committing the note than "ok" does, and a merged single-surface box with no internal borders reads as one control rather than two stacked ones. *(decided by: frontend-developer subagent)*
...
- [ ] Enlarged modal: translucent backdrop, full-size(r) screenshot, and a note editor using the same merged textarea + footer-bar style as the add-mode comment box, with a danger-styled **delete** button in the footer. **Decision:** Note edits autosave on blur or modal close (no explicit save button). **Rationale:** Autosave on blur/close is consistent with the document's stated principle "all changes autosave immediately" elsewhere; reduces modal friction and aligns with modern UX patterns for transient text editing. *(decided by: frontend-developer subagent)*
