...
Stored empty → returns 0 (no opinion), preserving the same backward-compat convention used everywhere else. The asymmetry between missing (−10) and extra (−7) is intentional: a stored heading we can't find on the candidate is the stronger mismatch signal (likely wrong section); an unexpected extra heading is weaker evidence (could just reflect page evolution since capture).
...
- **`headingPath` and `pageHeading` coexist instead of one replacing the other.** They answer different questions: `pageHeading` is fast and gives a single high-confidence signal (−80 mismatch); `headingPath` is set-based, more permissive (+5/−10/−7), and catches cases where the nearest heading happens to collide across contexts (e.g. a stepper sidebar that puts the same heading at the end of every step's preceding-headings list). The XPath is intentionally **not** chained across the full path — chaining many headings would fail on any single drift, so we use `headingPath` for scoring only and keep the single-heading-anchored XPath for lookup.
- **Text-match credit is conditional on broader-context agreement.** The `closestLabel` / `pageHeading` / `pageSubHeading` *match* bonuses are scaled by `headingPath` Jaccard similarity. This catches the failure mode where a heading text re-appears in a different section (a wizard summary listing prior step headings) — the narrow signals would otherwise pay full credit on a coincidence. Mismatches stay at full magnitude.
- **`isStableAttrValue` rejects framework-generated identifiers.** Numeric, UUID, long-hex, and **React `useId` patterns** (`:rXX:`) — the last one catches Cloudscape's `data-analytics-funnel-value="button:r9r:"` and similar component-library leakage, where the value regenerates per render.
